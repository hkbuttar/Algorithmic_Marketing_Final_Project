...
# verification profiles are NOT good substitutes.
#
# Consumes:
#   1.1 → data/processed/ulta_segmentation.csv
...
def _load_data():
    ...
    t0 = time.time()
    print("Loading Ulta recommendation data from CSVs...")

    _data["seg"] = pd.read_csv(PROCESSED_DIR / "ulta_segmentation.csv")
    ...
    _data["recs"] = pd.read_csv(PROCESSED_DIR / "ulta_recommendations.csv")
...
def _build_feature_blocks(df):
    ...
    # Ulta: verification provenance + helpfulness
    verif_cols = ["pct_verified_buyer", "pct_verified_reviewer",
                  "pct_would_recommend", "pct_has_disclosure"]
    verif_cols = [c for c in verif_cols if c in df.columns]
    struct_extra = df[verif_cols].fillna(0)
    if "helpfulness_rate" in df.columns:
        struct_extra = struct_extra.copy()
        struct_extra["helpfulness_rate"] = df["helpfulness_rate"].fillna(0)
    ...
    print(f"    A. Structural:  {structural.shape[1]} features "
          f"(incl. {len(verif_cols)} verification)")
...
def _compute_scores(sims, df):
    """Ulta weighting — structural (verification) gets higher weight for substitutes."""
    n = len(df)
    ...
    # Ulta substitutes: 25% structural (verification-aware)
    sub = (0.30 * content + 0.25 * sentiment + 0.25 * structural + 0.10 * price_sim + 0.10 * same_cat)
    sub *= (same_cat * 0.5 + 0.5)
    sub *= tw[np.newaxis, :]

    comp = (0.30 * content + 0.25 * sentiment + 0.25 * structural + 0.20 * cross_cat)
    comp *= cross_cat
    comp *= tw[np.newaxis, :]

    base = (0.50 * content + 0.30 * sentiment + 0.20 * structural)
    base *= (same_cat * 0.7 + 0.3)
    base *= tw[np.newaxis, :]
    ...
    for i in range(n):
        ...
        for prefix, matrix, top_n in [("sub", sub, TOP_N_SUB), ("comp", comp, TOP_N_COMP),
            ...
            for rank, j in enumerate(top_j):
                ...
                row[f"{prefix}_{rank+1}_verified_buyer"] = round(float(vb[j]), 4)
                row[f"{prefix}_{rank+1}_disclosure"] = round(float(disc[j]), 4)
        rows.append(row)
...
def run_pipeline():
    ...
    # Ulta-specific: verification alignment
    if "pct_verified_buyer" in df.columns:
        id_to_vb = dict(zip(df["product_id"].astype(str), df["pct_verified_buyer"]))
        gaps = []
        for _, r in recs_df.iterrows():
            src_vb = id_to_vb.get(str(r["product_id"]), 0) or 0
            for rk in range(1, TOP_N_SUB + 1):
                sid = r.get(f"sub_{rk}_id")
                if pd.notna(sid):
                    tgt_vb = id_to_vb.get(str(sid), 0) or 0
                    gaps.append(abs(src_vb - tgt_vb))
        if gaps:
            print(f"  Substitute verification gap: {np.mean(gaps)*100:.1f}pp avg")
...
def build_product_dashboard(product_id):
    ...
    def _extract(prefix, top_n):
        ...
        for rk in range(1, top_n + 1):
            ...
            items.append(dict(
                ...
                vb=vb_val, disc=disc_val, vtag=vtag,
            ))
        ...
        hover = [
            ...
            f"{it['reviews']:,} reviews · Score {it['score']:.4f}<br>"
            f"<b>{it['vtag']}</b>"
            for it in items
        ]
    ...
    if items:
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=colors_v[::-1], showlegend=False,
    ...
    # Full detail table with verification
    t_intent, t_brand, t_name, t_cat, t_price, t_score, t_verif = \
        [], [], [], [], [], [], []
    for prefix, ilabel, top_n in [("sub", "Substitute", TOP_N_SUB),
        ...
        for rk in range(1, top_n + 1):
            ...
            vb_v = float(row.get(f"{prefix}_{rk}_verified_buyer", 0) or 0)
            disc_v = float(row.get(f"{prefix}_{rk}_disclosure", 0) or 0)
            t_verif.append("Seeded" if disc_v > 0.2 else "Verified" if vb_v > 0.2 else "Low")
    ...
    fig.add_trace(go.Table(
        header=dict(
            values=["Type", "Brand", "Product", "Cat.", "Price", "Score", "Verif."],
            font=dict(size=10, color="white"), fill_color="#880e4f", align="left",
        ),
        cells=dict(
            values=[t_intent, t_brand, t_name, t_cat, t_price, t_score, t_verif],
            font=dict(size=9), fill_color=[fc] * 7, align="left",
        ),
    ), row=4, col=2)
    ...
    if src_disc > 0.20:
        verif_badge = f"⚠️ Seeded ({src_disc:.0%} disclosure)"
    elif src_vb > 0.20:
        verif_badge = f"✓ Verified ({src_vb:.0%} buyers)"
    else:
        verif_badge = "○ Low verification"
    ...
    kpi = (
        ...
        f"<span style='font-size:11px; color:#880e4f; font-weight:bold;'>"
        f"{verif_badge}</span>"
        f" <span style='font-size:10px; color:#999;'>"
        f"&nbsp;|&nbsp; Bars: "
        f"<span style='color:#27ae60;'>■</span> Organic "
        f"<span style='color:#e67e22;'>■</span> Seeded "
        f"<span style='color:#3498db;'>■</span> Low "
        f"<span style='color:red;'>---</span> Source &nbsp;|&nbsp; "
        f"Hover bars for details</span>"
    )
...
def show_dashboard():
    ...
    display(HTML(
        "<h2 style='text-align:center; color:#880e4f;'>"
        "Ulta Beauty — Product Recommendation Dashboard</h2>"
